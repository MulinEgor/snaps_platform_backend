...
class Repository:
    ...
    async def get(self, uuid: str) -> any:
        async with db_session() as session:
            ...
            return await table.find_unique(where={'uuid': uuid})

    async def get_all(self, filters: dict) -> list[any]:
        async with db_session() as session:
            table = self.get_table(session)
            return await table.find_many(where=filters)
    ...
    async def update(self, uuid: str, data: dict) -> any:
        async with db_session() as session:
            table = self.get_table(session)
            return await table.update(where={'uuid': uuid}, data=data)

    async def delete(self, uuid: str) -> any:
        async with db_session() as session:
            table = self.get_table(session)
            return await table.delete(where={'uuid': uuid})
